...
public class ImportService {
    ...
    public void importEntities(String kind, HashMap<String, Object>[] entities) throws InvalidPropertyException, AssignmentExistsException, InvalidNodeTypeException, NodeNotFoundException, ClassNotFoundException, NodeIDExistsException, NodeNameExistsException, NodeNameExistsInNamespaceException, IOException, ConfigurationException, SQLException, NullNameException, DatabaseException, NullTypeException, InvalidAssignmentException, UnexpectedNumberOfNodesException, AssociationExistsException, NoBaseIDException, PropertyNotFoundException, InvalidAssociationException, InvalidKeySpecException, NoSuchAlgorithmException {
        //create pc for kind
        OldNode pcNode = nodeService.createNode(NEW_NODE_ID, kind, PC.toString(), null);

        //create base OA for kind
        OldNode oaNode = nodeService.createNode(NEW_NODE_ID, kind, OA.toString(), null);
        ...
        for (HashMap<String, Object> map : entities) {
            ...
            OldNode entityNode = nodeService.createNode(NEW_NODE_ID, String.valueOf(id), OA.toString(), properties);
            ...
            for (String key : map.keySet()) {
                ...
                OldNode node = nodeService.createNode(NEW_NODE_ID, key, NodeType.O.toString(), properties);
    ...
    public void importSql(String host, int port, String schema, String username, String password, String session, long process)
        ...
        OldNode pcNode = nodeService.createPolicy(schema, properties, session, process);

        properties.clear();
                properties.put(Constants.SCHEMA_COMP_PROPERTY, Constants.SCHEMA_COMP_SCHEMA_PROPERTY);
                properties.put(Constants.DESCRIPTION_PROPERTY, "Base Object Attribute for " + schema);
        // create the schema object attribute node
        OldNode schemaNode = nodeService.createNodeIn(pcNode.getID(), schema, OA.toString(), properties, session, process);
        ...
        try {
            ...
            while(rs.next()){
                ...
                OldNode tableNode = nodeService.createNodeIn(schemaNode.getID(), tableName,
                ...
                OldNode columnsNode = nodeService.createNodeIn(tableNode.getID(), Constants
                ...
                String columnSql = "";
                HashMap<String, OldNode> columnNodes = new HashMap<>();
                while(rs1.next()){
                    ...
                    properties.clear();
                            properties.put(NAMESPACE_PROPERTY, tableName);
                    OldNode columnNode = nodeService.createNodeIn(columnsNode.getID(), columnName, OA.toString(),
                ...
                if(!columnSql.isEmpty()){
                    ...
                    OldNode rowsNode = nodeService.createNodeIn(tableNode.getID(), Constants.ROW_CONTAINER_NAME,
                            OA.toString(), properties, session, process);
                    ...
                    while(rs2.next()){
                        ...
                        OldNode rowNode = nodeService.createNode(NEW_NODE_ID, rowName, OA.toString(), properties);
                        /*OldNode rowNode = nodeService.createNode(rowsNode.getID(), NEW_NODE_ID, rowName, NodeType.OA
                                        .toString(),
                                properties);*/
                        ...
                        for(int i = 1; i <= rs2MetaData.getColumnCount(); i++){
                            //get column
                            String columnName = rs2MetaData.getColumnName(i);
                            OldNode columnNode = columnNodes.get(columnName);//nodeService.getNodeInNamespace(tableName, columnName, NodeType.OA, session, process);
                            ...
                            /*OldNode objectNode = nodeService.createNode(rowNode.getID(), NEW_NODE_ID, objectName, NodeType.O.toString(),
                                    properties);*/
                            OldNode objectNode = nodeService.createNode(NEW_NODE_ID, objectName, NodeType.O.toString(), properties);
    ...
    void createAssociation(long uaId, long targetID, HashSet<String> ops) throws ClassNotFoundException, SQLException, DatabaseException, InvalidPropertyException, IOException {
        ...
        getPIP().getAssociationsDAO().createAssociation(uaId, targetID, ops);

        //create association in nodes
        getPIP().getGraphDAO().getGraph().createAssociation(uaId, targetID, ops);
    }

    private OldNode getSuperUA() throws ClassNotFoundException, SQLException, DatabaseException, IOException, InvalidPropertyException, InvalidNodeTypeException, PropertyNotFoundException, NodeNotFoundException {
        HashSet<OldNode> nodesOfType = getPIP().getGraphDAO().getGraph().getNodesOfType(NodeType.UA);
        for(OldNode node : nodesOfType) {
